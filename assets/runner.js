...
var isGameRunning = false;  // Variable para controlar si el juego está en marcha

/**
 * Mueve al jugador a la posición proporcionada (izquierda, centro, derecha)
 * @param {int} position_index - Posición para mover al jugador
 */
...
/**
 * Controla los joysticks de Meta Quest 3 para mover la bolita de izquierda a derecha
 **/
AFRAME.registerComponent('controller-controls', {
  init: function () {
    var controllerEl = this.el;
    
    // Detecta el movimiento de los joysticks
    controllerEl.addEventListener('thumbstickmoved', (e) => {
      var joystick = e.detail;
      
      if (joystick) {
        // Si el joystick mueve más de un 10% hacia la izquierda o derecha
        if (joystick.x < -0.1) {
          movePlayerTo(0); // Mover al carril izquierdo
        } else if (joystick.x > 0.1) {
          movePlayerTo(2); // Mover al carril derecho
        } else {
          movePlayerTo(1); // Mover al carril central
        }
      }
    });
  }
...
/**
 * Detecta si el usuario presiona cualquier botón de los controladores para iniciar el juego
 **/
AFRAME.registerComponent('start-game-button', {
  init: function () {
    var controllerEl = this.el;
    
    // Detecta la presión de cualquier botón
    controllerEl.addEventListener('buttondown', (e) => {
      if (!isGameRunning) {
        startGame();  // Inicia el juego al presionar cualquier botón
      }
    });
  }
});
...
/**
 * Inicia el juego al presionar cualquier botón
 **/
function startGame() {
  isGameRunning = true;
  console.log("Juego iniciado");
  hideEntity(document.getElementById('start-menu'));
  setupScore();
  setupTrees();
...
/**
 * Agrega árboles aleatoriamente a diferentes carriles.
 **/
